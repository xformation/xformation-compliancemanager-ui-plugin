...
export default class QueryPanel extends React.Component<any, any> {
  ...
  suggestions() {
    ...
    Utils.postReq(config.SUGGEST_URL, 'query=' + val + '&cls=' + this.state.selEntity, (response: any, err: any) => {
      if (err) {
        this.setState({
  ...
  execute(query: any) {
    const params =
      'query=' + JSON.stringify(query) + '&cls=' + this.state.selEntity + '&pageNo=0&pageSize=10&notOnlyIds=true';
    Utils.postReq(config.ELS_QUERY, params, (response: any, err: any) => {
      if (err) {
        this.setState({
          elsQry: 'Request Failed with ' + err,
        });
